...
final class ___VARIABLE_productName___Wireframe: BaseWireframe {
    ...
    func pushFrom(_ parent: UINavigationController?, delegate: ___VARIABLE_productName___Delegate) {
        guard let viewController = self.createModule(delegate: delegate), let parent = parent else { return }
        self.presentedViewController = viewController
        parent.pushViewController(viewController, animated: true)
    }
    
    func presentIn(_ parent: UIViewController?, delegate: ___VARIABLE_productName___Delegate) {
        guard let viewController = self.createModule(delegate: delegate), let parent = parent else { return }
